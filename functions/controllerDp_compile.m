	% controllerDp_compile: Generate MEX-function controllerDp_mex from controllerDp.
	 
	% Script generated from project 'controllerDp.prj' on 17-Aug-2016.
	% 
	% See also CODER, CODER.CONFIG, CODER.TYPEOF, CODEGEN.

	% Call pattern for controllerDp:
	% [bestDischarge, bestCTG] = controllerDp(cfg, demForecast, pvForecast, battery, hourNow);

	%% Create configuration object of class 'coder.MexCodeConfig'.
	coder_cfg = coder.config('mex');
	coder_cfg.GenerateReport = true;
	...
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% demForecast, pvForecast arguments
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	ARGS{1}{2} = coder.typeof(0,[48 1],1);
	ARGS{1}{3} = coder.typeof(0,[48 1],1);
	...
	ARGS{1}{4}.cfg.bat.damageModel = coder.typeof('X',[1 10], 1);
	...
	ARGS{1}{4}.cfg.type = coder.typeof('X',[1 10], 1);
	...
	ARGS{1}{4}.statesInt = coder.typeof(0,[1 2000],1);
	ARGS{1}{4}.statesKwh = coder.typeof(0,[1 2000],1);
	...
	%% Invoke MATLAB Coder.
	codegen -config coder_cfg controllerDp -args ARGS{1}
